...
module.exports = function (app) {
  ...
  app.route('/api/issues/:project')
    ...
    .post(async function (req, res){
      ...
      console.log(req.params.project, req.body)
    ...
    .put(async function (req, res){
      ...
      console.log(newUpdates, missingId)

      if (newUpdates && !missingId) {
        const result = await issueModel.updateIssue( {_id: req.body._id }, update);
        if (result.n === 1) {
          console.log("result: 'successfully updated', '_id': ", req.body._id, update)
          return res.json({ result: 'successfully updated', '_id': req.body._id })
        } else {
          console.log("error: 'could not update', '_id': ", req.body._id, update)
          return res.json({ error: 'could not update', '_id': req.body._id })
        }
      } else if (missingId) {
        console.log("error: 'missing _id', '_id': ", req.body._id, update)
        return res.json({ error: 'missing _id' })
      } else if (!newUpdates) {
        console.log("error: 'no update field(s) sent', '_id': ", req.body._id, update)
