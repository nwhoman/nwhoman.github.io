...
app.post('/api/users/:_id/exercises', async (req, res) => {
  ...
  console.log(user[0])
  const exercise = await constructs.createExercise(user[0].username, req.body.description, req.body.duration, constructs.formatDate(req.body.date), req.params._id)
...
app.get('/api/users/:_id/logs', (req, res) => {
  ...
  .then(exerciseList => {
    ...
    if (req.query['to']){
      ...
      log = log.filter(function(item) {
        if (Date.parse(item["date"]) <= toDate){
          console.log(typeof item["date"])
          return item["date"]
        }
      })
...
app.get('/api/delete', (req, res) => {
  ...
  .then(items => items.forEach(item => {
    ...
    constructs.getAllExercises({ username: { $regex: "fcc_test", $options: "i"}})
    .then(items => items.forEach(item => {
      console.log(item)
      constructs.de(item.id)
      count++
}));
