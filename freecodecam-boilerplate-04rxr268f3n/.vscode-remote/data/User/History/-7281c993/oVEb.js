...
const createExercise = async (username, description, duration, date = Date.now(), userId) => {
    if (date == '') {
        date = Date.now();
    }
...
const deleteExercises = async (_id, update) => {
    const result = await ExerciseModel.deleteOne({_id: _id}, update);
    return result.deletedCount;
};
...
export { ExerciseModel, UserModel, createUser, createExercise, getUserById, getAllUsers, getAllExercises, deleteUsers, deleteOne, formatDate, deleteExercises };