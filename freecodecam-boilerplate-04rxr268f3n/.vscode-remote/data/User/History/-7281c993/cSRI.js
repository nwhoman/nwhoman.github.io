...
const getAllExercises = async (userId) => {
  console.log(userId);
  const query = await ExerciseModel.find({ userId: userId }).exec();
  console.log(query);
  return query;
};
...
const deleteUsers = async (update) => {
    ...
    query.forEach(item => {
        ...
        if (item.username.slice(0, 8) == "fcc_test") {
            console.log(item.id, item.username);
            const result = UserModel.deleteOne({ _id: item.id }, update);
            count++
        }
    });
    console.log(count);
...
export { ExerciseModel, UserModel, createUser, createExercise, getUserById, getAllUsers, getAllExercises, deleteUsers };