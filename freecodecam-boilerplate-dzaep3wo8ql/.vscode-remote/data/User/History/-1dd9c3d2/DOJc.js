class SudokuSolver {
  ...
  checkRowPlacement(puzzleString, row, column, value) {
    ...
    return this.checkColPlacement(puzzleString, rowValue, column, value)
  }
  ...
  checkColPlacement(puzzleString, row, column, value) {
    ...
    return this.checkRegionPlacement(puzzleString, row, column, value)
  }
        ...
        for (let i=row_start; i < row_start + 3; i++) {
          for (let j=col_start; j < col_start + 3; j++) {
            ...
            if (puzzleString[i*9 + j] === value) {
              return { valid: false, conflict: [ 'zone' ] }
            }
          }
        }
    
    let start = puzzleString.slice(0, row*9 + column) + value;
    let end = puzzleString.slice(row*9 + column + 1)
    puzzleString = start + end;
    return puzzleString
  }
